...
export default function WebhookModal({ webhook, onClose, canReplay }: WebhookModalProps) {
  ...
  const tabs = [
    { key: 'overview' as const, label: t('webhook.overview'), icon: Globe },
    { key: 'headers' as const, label: t('webhook.headers'), icon: Hash },
    { key: 'body' as const, label: t('webhook.body'), icon: FileJson },
    { key: 'query' as const, label: t('webhook.query'), icon: ArrowRightLeft },
    ...(webhook.statusCode ? [{ key: 'response' as const, label: t('webhook.response'), icon: Terminal }] : []),
    ...(canReplay ? [{ key: 'replay' as const, label: t('webhook.replay'), icon: Play }] : []),
  ];

  const handleReplay = async () => {
    setReplayJsonError('');
    let parsedHeaders: any, parsedBody: any, parsedQuery: any;
    try { parsedHeaders = JSON.parse(replayHeaders || '{}'); } catch { setReplayJsonError(`${t('webhook.headers')}: Invalid JSON`); return; }
    try { parsedBody = replayBody.trim() || undefined; try { parsedBody = JSON.parse(replayBody); } catch { /* raw */ } } catch { setReplayJsonError(`${t('webhook.body')}: Invalid`); return; }
    try { parsedQuery = JSON.parse(replayQuery || '{}'); } catch { setReplayJsonError(`${t('webhook.query')}: Invalid JSON`); return; }
      ...
      <div className="relative w-full max-w-4xl max-h-[90vh] bg-slate-900 rounded-2xl border border-slate-700 shadow-2xl flex flex-col overflow-hidden">
        ...
        <div className="flex items-center justify-between p-5 border-b border-slate-800 bg-slate-800/50">
          <div className="flex items-center gap-3">
            ...
            {webhook.isReplay && (
              <span className="flex items-center gap-1 text-xs text-purple-400 bg-purple-500/10 border border-purple-500/20 px-2 py-1 rounded-md"><Clock className="w-3 h-3"/>{t('webhook.replay')}</span>
            )}
          </div>
        ...
        <div className="flex-1 overflow-auto p-5">
          {activeTab==='overview'&&(
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <InfoCard label={t('webhook.id')} value={webhook.id} monospace copy onCopy={copyId} copied={copied} />
              <InfoCard label={t('webhook.timestamp')} value={new Date(webhook.createdAt).toLocaleString()} />
              <InfoCard label="Relative Time" value={formatDate(webhook.createdAt)} />
              <InfoCard label={t('webhook.ip')} value={webhook.ip} monospace />
              <InfoCard label="Source" value={webhook.source || 'custom'} />
              <InfoCard label={t('webhook.userAgent')} value={webhook.userAgent || '—'} />
              <InfoCard label={t('webhook.size')} value={formatBytes(bodySize)} />
          ...
          {activeTab==='headers'&&(
            <div className="bg-slate-800/50 rounded-xl border border-slate-800 overflow-hidden">
              <table className="w-full text-sm">
                <thead><tr className="border-b border-slate-700 bg-slate-800/80"><th className="text-left text-xs font-semibold text-slate-500 uppercase tracking-wider px-4 py-2.5 w-1/3">{t('webhook.headers')}</th><th className="text-left text-xs font-semibold text-slate-500 uppercase tracking-wider px-4 py-2.5">{t('common.value')}</th></tr></thead>
                <tbody>
                  ...
                  )):<tr><td colSpan={2} className="px-4 py-8 text-center text-slate-500 text-sm">{t('webhook.noHeaders')}</td></tr>}
                </tbody>
              </table>
            </div>
          )}

          {activeTab==='body'&&(<div>{webhook.body?<JsonViewer data={webhook.body}/>:<div className="flex flex-col items-center justify-center py-16 text-slate-500"><FileJson className="w-10 h-10 mb-3 opacity-30"/><p className="text-sm">{t('webhook.noBody')}</p></div>}</div>)}

          {activeTab==='query'&&(
            <div className="bg-slate-800/50 rounded-xl border border-slate-800 overflow-hidden">
              <table className="w-full text-sm">
                <thead><tr className="border-b border-slate-700 bg-slate-800/80"><th className="text-left text-xs font-semibold text-slate-500 uppercase tracking-wider px-4 py-2.5 w-1/3">{t('webhook.query')}</th><th className="text-left text-xs font-semibold text-slate-500 uppercase tracking-wider px-4 py-2.5">{t('common.value')}</th></tr></thead>
                <tbody>
                  ...
                  )):<tr><td colSpan={2} className="px-4 py-8 text-center text-slate-500 text-sm">{t('webhook.noQuery')}</td></tr>}
                </tbody>
              </table>
            </div>
          )}
          ...
          {activeTab==='response'&&(
            <div className="space-y-4">
              ...
              {webhook.responseBody ? (
                <div><h4 className="text-xs font-semibold text-slate-500 uppercase tracking-wider mb-2">{t('webhook.response')}</h4><div className="bg-slate-800/50 rounded-xl border border-slate-800 p-4 overflow-auto max-h-[50vh]"><pre className="text-sm text-slate-300 font-mono whitespace-pre-wrap">{webhook.responseBody}</pre></div></div>
              ):(<div className="flex flex-col items-center justify-center py-16 text-slate-500"><MessageSquare className="w-10 h-10 mb-3 opacity-30"/><p className="text-sm">{t('webhook.noBody')}</p></div>)}
            </div>
          )}
          ...
          {activeTab==='replay'&&canReplay&&(
            <div className="space-y-5 max-w-2xl">
              ...
              <div className="flex items-center gap-3">
                <span className={`px-2.5 py-1 rounded-md text-sm font-mono font-bold border ${methodColor(webhook.method)}`}>{webhook.method}</span>
                <span className="text-sm text-slate-400">{t('webhook.replay')}</span>
              </div>

              {/* Colorful sub-tabs */}
              <div className="flex gap-2">
                {([
                  {id:'url', label:t('webhook.targetUrl'), icon:Globe, color:'text-emerald-400', bg:'bg-emerald-500/10', border:'border-emerald-500/20', bar:'bg-emerald-500/30'},
                  {id:'headers', label:t('webhook.headers'), icon:Hash, color:'text-sky-400', bg:'bg-sky-500/10', border:'border-sky-500/20', bar:'bg-sky-500/30'},
                  {id:'body', label:t('webhook.body'), icon:FileJson, color:'text-purple-400', bg:'bg-purple-500/10', border:'border-purple-500/20', bar:'bg-purple-500/30'},
                  {id:'query', label:t('webhook.query'), icon:Link2, color:'text-amber-400', bg:'bg-amber-500/10', border:'border-amber-500/20', bar:'bg-amber-500/30'},
                ] as const).map(tab => (
              ...
              {replaySubTab==='url'&&(
                <div className="space-y-2">
                  <label className="text-sm font-semibold text-emerald-400 flex items-center gap-2"><Globe className="w-4 h-4"/>{t('webhook.targetUrl')}</label>
                  ...
                  <p className="text-xs text-slate-500">{t('webhook.replay')}</p>
                </div>
              )}
              ...
              {replaySubTab==='headers'&&(
                <div className="space-y-2">
                  <div className="flex items-center justify-between">
                    <label className="text-sm font-semibold text-sky-400 flex items-center gap-2"><Hash className="w-4 h-4"/>{t('webhook.headers')} <span className="text-slate-600 font-normal text-xs">JSON</span></label>
                    <button onClick={()=>setReplayHeaders(JSON.stringify(webhook.headers||{},null,2))} className="text-xs text-slate-500 hover:text-sky-400 flex items-center gap-1 transition-colors"><RotateCcw className="w-3 h-3"/>{t('webhook.resetAll')}</button>
                  </div>
              ...
              {replaySubTab==='body'&&(
                <div className="space-y-2">
                  <div className="flex items-center justify-between">
                    <label className="text-sm font-semibold text-purple-400 flex items-center gap-2"><FileJson className="w-4 h-4"/>{t('webhook.body')}</label>
                    <button onClick={()=>setReplayBody(formatReplayBody(webhook.rawBody, webhook.body))} className="text-xs text-slate-500 hover:text-purple-400 flex items-center gap-1 transition-colors"><RotateCcw className="w-3 h-3"/>{t('webhook.resetAll')}</button>
                  </div>
              ...
              {replaySubTab==='query'&&(
                <div className="space-y-2">
                  <div className="flex items-center justify-between">
                    <label className="text-sm font-semibold text-amber-400 flex items-center gap-2"><Link2 className="w-4 h-4"/>{t('webhook.query')} <span className="text-slate-600 font-normal text-xs">JSON</span></label>
                    <button onClick={()=>setReplayQuery(JSON.stringify(webhook.query||{},null,2))} className="text-xs text-slate-500 hover:text-amber-400 flex items-center gap-1 transition-colors"><RotateCcw className="w-3 h-3"/>{t('webhook.resetAll')}</button>
                  </div>
          ...
          {activeTab==='replay'&&!canReplay&&(
            <div className="flex flex-col items-center justify-center py-16 text-slate-500">
              ...
              <p className="text-lg font-medium">{t('webhook.replayRequiresPro')}</p>
              <p className="text-sm text-slate-600 mt-1">{t('webhook.replayRequiresPro')}</p>
            </div>
          )}
        </div>
      </div>
